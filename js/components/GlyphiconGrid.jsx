...
module.exports = class GlyphiconGrid extends React.Component {
  ...
  render() {
    ...
    let items = this.glyphicons;
    if (this.state.filter !== '') {
      items = this.glyphicons.filter( (glyphName)=> glyphName.includes(this.state.filter) );
    }
    ...
    return (
      ...
      <div className="GlyphiconGrid">
        ...
        <div className="input-group" style={{marginBottom: 5}}>
          ...
          <input type="text" value={this.state.filter} onChange={this.onChange.bind(this)} className="form-control" placeholder="Filter"/>
        </div>
        <ul>
          {items.map(gridItem)}
        </ul>
